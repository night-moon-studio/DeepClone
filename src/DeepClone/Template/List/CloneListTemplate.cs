...
{
    public class CloneListTemplate
    {
        ...
        {
            return type is IList  && type != typeof(object) && type != typeof(string) ;
        }
        ...
        public Delegate TypeRouter(Model.BuilderInfo info)
        {
            ...
            if (info.DeclaringType.IsGenericType)
            {
                ...
                // 值类型
                if (info.DeclaringType.IsOnceType())
                {
            ...
            var action = FastMethodOperator.New
                            .Using("DeepClone")
                            .Using("Natasha")
                            .Using(info.DeclaringType.GetGenericArguments()[0].GetDevelopName())// List 中的类型
                            .Using(typeof(IList))
